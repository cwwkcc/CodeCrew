...
.button--large {
  --btn-pad: var(--spacing-md) var(--spacing-lg);
}
```
...
```css
/* WRONG: no fallback — if the variable is undefined (typo, or not yet
   set in this scope), the property gets NO value at all, not a default */
.card {
  background: var(--suface-color);
} /* typo: "suface" — silently
                                                  falls through to nothing */

/* CORRECT: always give var() a fallback for anything that isn't
   guaranteed to exist in every context this rule can run in */
.card {
  background: var(--surface-color, #ffffff);
}
```

- **Using `var()` where a static value is required, like inside a media query condition.** `@media (min-width: var(--breakpoint-md)) { }` doesn't work — custom properties are a _value_ substitution mechanism for declarations, not a general-purpose constant system the way `$variables` are in Sass. Media query breakpoints still need literal values (or a build-time preprocessor step).
- **Defining every custom property on `:root` even when it should vary by component or theme.** A single global `--color-*` palette is a great start, but scoping a property to a specific selector (`.dark-theme { --surface-color: #111; }`) is how you get real theming — a variable redefined inside a scope cascades to everything inside it, without touching the global default.
